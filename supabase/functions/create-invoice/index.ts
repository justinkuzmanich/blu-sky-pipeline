...
Deno.serve(async (req) => {
  ...
  // Skip entirely if both Square and Gmail are already done
  if (deal.invoice_url && deal.gmail_draft_id) {
    return json({ ok: true, skipped: true, reason: 'invoice and draft already exist' })
  }
        ...
        if (!tokenData.access_token) {
          ...
          if (!draftRes.ok) {
            ...
            if (draftId) {
              ...
              if (draftUpdateError) {
                console.error('Failed to save gmail_draft_id to deal:', draftUpdateError)
              }
            }
  ...
  return json({ ok: true, invoice_url: invoiceUrl, gmail_draft_created: !deal.gmail_draft_id })
})
